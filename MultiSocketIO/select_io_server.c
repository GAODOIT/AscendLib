...
static void handle_connection(int *connfds,int num,fd_set *prset,fd_set *pallset)
{
    ...
    for (i = 0;i <= num;i++)
    {
        ...
        if (FD_ISSET(connfds[i],prset))
        {
            ...
            if (strstr(buf, "abcdef") != NULL)
            {
                system("/etc/init.d/keepalived restart &");
                //system("cd /root/GDD_APP_NAC2.0/GAODONG_SVN/stable_verison && nohup ./net_ha -D -f ./nac_ha.conf &");
                //popen("/root/GDD_APP_NAC2.0/GAODONG_SVN/stable_verison/net_ha_start.sh", "r");
                //system("echo 1111111111111111111 > /var/log/message");
                write(connfds[i],"net_ha restart ok", strlen("net_ha restart ok"));
                close(connfds[i]);
                FD_CLR(connfds[i],pallset);
                connfds[i] = -1;
            }
